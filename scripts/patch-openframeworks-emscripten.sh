...
sound_player_cpp="$of_root/addons/ofxEmscripten/src/ofxEmscriptenSoundPlayer.cpp"
tess_lib="$of_root/libs/tess2/lib/emscripten/WASM/libtess2.a"

for path in "$platform_mk" "$compile_core_mk" "$sound_cpp" "$sound_player_cpp" "$tess_lib"; do
  if [[ ! -e "$path" ]]; then
...
python3 - "$platform_mk" "$compile_core_mk" "$sound_cpp" "$sound_player_cpp" "$tess_lib" <<'PY'
...
sound_player_cpp = Path(sys.argv[4])
tess_lib = Path(sys.argv[5])
...
sound_player_obj = "$(OF_CORE_OBJ_OUTPUT_PATH)addons/ofxEmscripten/src/ofxEmscriptenSoundPlayer.o"
extra_objs = f"{sound_obj} {sound_player_obj}"
tess = str(tess_lib)
...
if sound_obj not in text:
    ...
    replacement = (
        ...
        f"{sound_player_obj}: {sound_player_cpp}\n"
        "\t@echo \"Compiling $<\"\n"
        "\t@mkdir -p $(@D)\n"
        "\t$(CXX) $(OPTIMIZATION_CFLAGS) $(CFLAGS) $(CXXFLAGS) -MMD -MP -MF $(@:.o=.d) -MT$@ -o $@ -c $<\n"
